...
public class HandleObjects {
    ...
    private static AbstractJdbcType<?> getType(Class<?> elementClass) throws SQLException {
        ...
        if (type==null) {
            throw new SQLRecoverableException(String.format("unsupported Collection element type:  '%s' for CQL", elementClass));
        }
        return type;
    }
    ...
    private static final Boolean objectToBOOLEAN(Class<? extends Object> objectClass, Object object) {
        ...
        if (objectClass == Integer.class) {
            ...
            return ((BigInteger) object).intValue() == 0 ? false : true;
        } else if (objectClass == Long.class) {
            return ((Long) object) == 0 ? false : true;
        } else if (objectClass == Short.class) {
            return ((Short) object) == 0 ? false : true;
        } else if (objectClass == Byte.class) {
            return ((Byte) object) == 0 ? false : true;
        }

        // Floating ones need to just pass the integer part
        else if (objectClass == Double.class) {
            return Integer.valueOf(((Double) object).intValue()) == 0 ? false : true;
        } else if (objectClass == Float.class) {
            return Integer.valueOf(((Float) object).intValue()) == 0 ? false : true;
        } else if (objectClass == BigDecimal.class) {
            return Integer.valueOf(((BigDecimal) object).intValue()) == 0 ? false : true;
        } else {
            return null; // this should not happen
        }
    }
