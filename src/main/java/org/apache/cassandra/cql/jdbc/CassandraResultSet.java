...
class CassandraResultSet extends AbstractResultSet implements CassandraResultSetExtras {
    ...
    private final Boolean getBoolean(TypedColumn column) throws SQLException {
        ...
        if (value instanceof Integer) {
            return Boolean.valueOf(((Integer) value) == 0 ? false : true);
        }

        if (value instanceof Long) {
            return Boolean.valueOf(((Long) value) == 0 ? false : true);
        }

        if (value instanceof BigInteger) {
            return Boolean.valueOf(((BigInteger) value).intValue() == 0 ? false : true);
        }
    ...
    private Long getLong(TypedColumn column) throws SQLException {
        ...
        if (value instanceof BigInteger) {
            return getBigInteger(column).longValue();
        }
    ...
    private TypedColumn createColumn(Column column) {
        ...
        if (validator == null) {
            if (valueType.equals("org.apache.cassandra.db.marshal.TimestampType")) {
                ...
                if (collectionClass.endsWith("ListType")) {
                    type = CollectionType.LIST;
                } else if (collectionClass.endsWith("SetType")) {
                    type = CollectionType.SET;
                } else if (collectionClass.endsWith("MapType")) {
